...
The most straightforward way to ensure your workloads can work on a restricted namespace, particularly in 4.12, is by labeling the namespaces where they should run by enforcing the restricted policy and verifying if they're admit and are  successfully running.
You may also check whether they're assigned the restricted-v2 SCC in 4.11. We recommend to validate and ensure the desired behavior by doing functionality end to end testing. 
