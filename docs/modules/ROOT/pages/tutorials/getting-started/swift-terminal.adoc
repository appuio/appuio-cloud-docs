= Getting Started on {product} with the Terminal using Swift
...
If you are not familiar with issuing commands on a terminal session, you might want to try the xref:tutorials/getting-started/swift-web.adoc[Getting Started with Swift on the OpenShift Web Console] guide instead.
