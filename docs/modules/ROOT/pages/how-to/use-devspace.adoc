...
The previous step generates a new file in your project called `devspace.yaml`.
...
. Finally, apply a few more `dev:replacePods:patches`, going from this:
+
[source,yaml]
--
  ...
  replacePods:
    ...
    persistPaths: # <2>
      - path: /app
        skipPopulate: true
--
<1> New `op` patch, adding environment values.
<2> New `persistPath` entry.
+
The `skipPopulate: true` is needed because otherwise an init container tries to copy stuff around, which leads to an `Operation not permitted` error.
...
and selecting the "via localhost" option.
