...
.URL length warning
====
Make sure that the total length of the prefix string `[YOUR_USERNAME]-fortune-go` used in the `Ingress` YAML above *isn't longer than 63 characters*. This is due for two reasons:

. As per the https://datatracker.ietf.org/doc/html/rfc1035#section-2.3.4[DNS RFC], each label (for example the prefix in this case) must be <= 63 characters (octets to be exact), and the whole domain name must be <= 255 octets.
...
IMPORTANT: It could be possible that GitHub Actions aren't automatically enabled in your repository, because it has been imported from another location. In that case, you can enable them by going to the menu:Settings[Actions > General] screen and selecting  the "Allow all actions" option.
...
. Create a file named `master.yml` with the following contents:
+
[source,yaml]
--
name: ci
