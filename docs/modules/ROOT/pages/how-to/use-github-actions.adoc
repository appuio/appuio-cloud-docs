...
.URL length warning
====
Make sure that the total length of the prefix string `[YOUR_USERNAME]-fortune-go` used in the `Ingress` YAML above is *not longer than 63 characters*. This is due for two reasons:
...
IMPORTANT: It could be possible that GitHub Actions are not automatically enabled in your repository, because it has been imported from another location. In that case, you can enable them by going to the menu:Settings[Actions > General] screen and selecting  the "Allow all actions" option.
