...
IMPORTANT: This guide is provided as a courtesy for {product} users. For more information about K8up, please refer to its https://k8up.io/[official documentation].

== Requirements

To follow this guide, please make sure that you have the following tool installed:
...
K8up is powered by the https://restic.net/[Restic] backup tool, and is thus able to store backups in any S3-compatible storage. For the purposes of this guide, you can quickly install a https://min.io/[MinIO] instance in your project.

IMPORTANT: This setup is only for demo purposes. In production settings, it is recommended to use an S3 provider outside of the cluster, such as https://aws.amazon.com/s3/[Amazon S3], https://www.exoscale.com/object-storage/[Exoscale Object Storage], a secure MinIO instance in another {product} zone, or similar.
...
A few seconds after applying this `Backup` object, you will see in the MinIO GUI that the Restic repository has been successfully initialized and populated.

=== 5. Restore the backup locally

Using the `restic` command line tool, we will now restore the backup locally on your system. Create and source a `environment.sh` script that exports the required environment variables used by https://restic.net/[restic]:
